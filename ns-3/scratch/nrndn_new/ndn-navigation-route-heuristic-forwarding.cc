...
void NavigationRouteHeuristic::OnDelete_RSU(Ptr<Face> face, Ptr<Interest> deletepacket)
{
	...
	else // it is from nodes behind
	{
		...
		if (idIsInPriorityList)
		{
			...
			std::string currentroute = routes[0];
			...
			forwardRoute = forwardRoute.substr(currentroute.size() + 1);
...
void NavigationRouteHeuristic::OnInterest_RSU(Ptr<Face> face, Ptr<Interest> interest)
{
	...
	//cout<<endl<<"(forwarding.cc-OnInterest_RSU)At Time "<<Simulator::Now().GetSeconds()<<" 当前RSUId为 "<<myNodeId<<" 源节点 "<<nodeId<<" 转发节点 "<<forwardId<<" seq "<<seq;
	//cout<<" 实际转发路线为 "<<forwardRoute<<endl;
...
void NavigationRouteHeuristic::OnData_Car(Ptr<Face> face, Ptr<Data> data)
{
	...
	if (m_dataSignatureSeen.Get(data->GetSignature()))
	{
		//cout<<"该数据包已经被发送"<<endl;
		//2018.1.24 从缓存中删除数据包
		//2019.1.2 低优先级的数据包不会被缓存
		if ((priority < 2) && msgdirection.first && msgdirection.second < 0)
		{
	...
	else
	{
		if (isDuplicatedData(nodeId, signature))
		{
			cout << "(forwarding.cc-OnData_Car) 该数据包从前方得到，重复，丢弃" << endl;
			ExpireDataPacketTimer(nodeId, signature);
			return;
		}
		...
		if (idIsInPriorityList)
		{
			...
			{
				cout << "(OnData_Car) 当前距离 " << currentDistance << " 已经超过了有效距离 " << totalDistance << " signature " << signature << endl;
...
//2018.12.28 获取虚拟PIT的全部后方路段
std::unordered_set<std::string> NavigationRouteHeuristic::getFakeInterestedRoutes(Ptr<pit::Entry> WillFake)
{
...
std::unordered_set<std::string> NavigationRouteHeuristic::getAllInterestedRoutes(Ptr<pit::Entry> Will, Ptr<pit::Entry> WillSecond)
{
	...
	if (Will)
	{
		cout << "(forwarding.cc-getAllInterestedRoutes) 主PIT中感兴趣的路段为 ";
	...
	if (WillSecond)
	{
		cout << "(forwarding.cc-getAllInterestedRoutes) 副PIT中感兴趣的路段为 ";
...
void NavigationRouteHeuristic::OnData_RSU_RSU(const uint32_t remoteId, Ptr<Data> data)
{
	...
	//低优先级的数据包不会被缓存
	if ((priority < 2) && m_dataSignatureSeen.Get(data->GetSignature()) && (forwardLane != ""))
	{
	...
	if (pri.empty())
	{
		cout << "(forwarding.cc-OnData_RSU_RSU) 数据包 " << data->GetSignature() << "转发优先级列表为空" << endl;
		return;
	}
...
void NavigationRouteHeuristic::OnData_RSU(Ptr<Face> face, Ptr<Data> data)
{
	...
	if (msgdirection.first && msgdirection.second <= 0) // 数据包位于当前路段后方
	{
		...
		if ((dataType == "vehicle") && (currentDistance > totalDistance))
		{
			cout << "(OnData_RSU) 当前距离 " << currentDistance << " 超过了有效距离。数据包来自于后方 " << signature << endl;
		}
	...
	else
	{
		if (pri.empty())
		{
			cout << "(forwarding.cc-OnData_RSU) 数据包 " << data->GetSignature() << "转发优先级列表为空" << endl;
			return;
		}
		...
		if (idIsInPriorityList)
		{
			...
			if ((dataType == "vehicle") && (currentDistance > totalDistance))
			{
				cout << "(OnData_RSU) 当前距离 " << currentDistance << " 已经超过了有效距离" << totalDistance << " signature " << signature << endl;
				m_sendingDataEvent[nodeId][signature] =
					Simulator::Schedule(sendInterval,
										&NavigationRouteHeuristic::BroadcastStopDataMessage, this, data);
				return;
...
void NavigationRouteHeuristic::ProcessHelloRSU(Ptr<Interest> interest)
{
	...
	else
	{
		...
		for (; itroutes_behind_pre != routes_behind_pre.end(); ++itroutes_behind_pre)
		{
			...
			if (itroutes_behind == routes_behind.end())
			{
				cout << "(forwarding.cc-ProcessHelloRSU) 后方路段数目不变，但是有新路段" << endl;
				nbchangeroad = true;
				break;
			}
			else
	...
	if (behind_change_mode > 1 && m_cs->GetDataSize() > 0)
	{
		...
		//2018.12.27
		std::unordered_set<std::string> interestDataName = m_nrpit->GetDataNameandLastRoute(routesCollection);

		std::map<uint32_t, Ptr<const Data>> datacollection = m_cs->GetData(interestDataName);
		if (!datacollection.empty())
		{
			SendDataInCache(datacollection);
		}
