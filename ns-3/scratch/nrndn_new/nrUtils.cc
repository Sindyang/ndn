...
nrUtils::TransmissionDelayMap nrUtils::HighPriorityTransmissionDelayRecord;
nrUtils::TransmissionDelayMap nrUtils::MediumPriorityTransmissionDelayRecord;
nrUtils::TransmissionDelayMap nrUtils::LowPriorityTransmissionDelayRecord;
...
std::pair<uint32_t, uint32_t> nrUtils::GetNodeSizeAndInterestNodeSize(
	const double &x, const double &y, uint32_t signature, const std::string &lane, const std::string &dataType, const uint32_t &distance)
{
	uint32_t nodeSize = 0;
	...
	for (it = c.Begin(); it != c.End(); ++it)
	{
		...
		if (producer->IsInterestLane(x, y, lane, dataType, distance))
		{
			++interestSize;
