...
void nrProducer::addAccident()
{
	...
	count = 0 * totalCount;
	UniformVariable nrndnew(start + totalTime * 0.25, end);
	while (count--)
	{
		t = nrndnew.GetValue();
		if (!m_accidentList.count(t))
...
void nrProducer::addAccident(double iType)
{
	...
	for (double dTime = start; dTime < end; dTime += iType)
	{
...
bool nrProducer::IsInterestLane(const std::string &lane)
{
	...
	//cout<<"nrProducer.cc Time now: "<<Simulator::Now().GetSeconds()<<endl;
