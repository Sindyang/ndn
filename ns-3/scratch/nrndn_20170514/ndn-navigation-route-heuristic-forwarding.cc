...
void NavigationRouteHeuristic::OnInterest_Car(Ptr<Face> face,Ptr<Interest> interest)
{
	...
	if(Face::APPLICATION==face->GetFlags())
	{
		...
		if(pri.empty())
		{
			...
			Simulator::Schedule(MilliSeconds(m_uniformRandomVariable->GetInteger(0,120)),&NavigationRouteHeuristic::CachingInterestPacket,this,interest->GetNonce(),interest);
			return;
		}
		...
		Simulator::Schedule(MilliSeconds(m_uniformRandomVariable->GetInteger(0,120)),
	...
	for(nb = m_nb.getNb().begin();nb != m_nb.getNb().end();++nb)
	{
		...
		if(nb->first >= numsofvehicles)
		{
			...
			for(;it != interestRoutes.end();it++)
			{
				...
				{
					cout<<"(forwarding.cc-RSUGetPriorityListOfData) 感兴趣路段为 "<<*it<<" RSU交点ID为 "<<junction<<endl;
					std::pair<bool,double> result = m_sensor->RSUGetDistanceWithRSU(nb->first,nb->second.m_lane);
		...
		else
		{
			...
			{
				cout<<"(forwarding.cc-RSUGetPriorityListOfData) 车辆 "<<nb->first<<" 所在路段为 "<<nb->second.m_lane<<" 不在感兴趣路段中"<<endl;
			}
		}
	}
